 ...
 *
 * Rate-limit package → ADR-055 (`src/rate-limiting`); audit persistence →
 * `src/audit-logging/` (ADR-058);
 * pen-test smoke / full hardening → ARD-020. Next.js route matrix wiring still caller-side.
 */

import { SECURITY_API_RULES } from "../api-standards/index.js";
import { EXCLUSIVE_SQL_ORM } from "../drizzle-orm-strategy/index.js";
import {
  SECRET_POLICY,
  AUTH_SECRET_ENV,
...
export const SECURITY_DECISION = {
  ...
  auditImplementationPackage: "src/audit-logging/",
  penSmokeArd: "ARD-020",
  apiProtectionAdr: "ADR-077",
  apiProtectionPackage: "src/api-protection",
} as const;
...
export const TRANSPORT_COOKIE_CONTROLS = {
  ...
  cookieContract: "src/nextauth-jwt",
} as const;
 ...
 * Deepened in ADR-077 (`src/api-protection`).
 */
