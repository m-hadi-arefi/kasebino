 ...
 *
 * Cache-aside helpers → `src/cache-aside` (ADR-052).
 * Key/TTL standards → `src/cache-keys` (ADR-053).
 * Invalidation event→key maps → `src/cache-invalidation` (ADR-054).
 * Rate-limit package → `src/rate-limiting` (ADR-055).
 *
...
export const FAILURE_MODES = {
  ...
  authAndOtpRateLimits: {
    ...
    detailAdr: "ADR-055",
    implementation: "src/rate-limiting",
...
export const DEFERRED_PLACEMENT = {
  architectureContract: "src/redis-architecture/",
  clientStub: "src/infrastructure/redis/client.ts",
  techFolderConvention: "src/shared/infrastructure/redis",
  rateLimitPackage: "src/rate-limiting/",
  cacheAsidePackage: "src/cache-aside/",
...
export const REDIS_REQUIREMENTS = {
  ...
  /** Key/TTL standards realized in ADR-053 `src/cache-keys`. */
  keyNamespaceDefer053: false,
  keysTtlImplementedAdr053: true,
  /** Fail policies realized in ADR-055 `src/rate-limiting`. */
  failPolicyNotesDefer055: false,
  rateLimitImplementedAdr055: true,
  /** Cache-aside helpers realized in ADR-052 `src/cache-aside`. */
  noCacheAsideInThisAdr: false,
  cacheAsideImplementedAdr052: true,
  /** Event→key invalidation realized in ADR-054 `src/cache-invalidation`. */
...
export function assertKeyNamespaceDeferred(detailAdr: string): void {
  ...
  if (KEY_NAMESPACE.implementation !== "src/cache-keys/") {
    throw new Error(
