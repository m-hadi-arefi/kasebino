...
export const MERCHANT_AUTH_DECISION = {
  ...
  /** Realized by ADR-033 — `src/infrastructure/auth/contracts/nextauth-jwt` + identity Auth.js stub. */
 ...
 * Enforcement: `src/infrastructure/security/rate-limiting` (ADR-055); fail-closed when Redis down (ADR-051).
 */
export const MERCHANT_OTP_RATE_LIMIT = {
  otpRequestsPerMinute: 3,
  authRoutesPerMinute: 5,
  redisKeyHint: "mos:{env}:rl:otp:{phone|ip}",
  enforcement: "src/infrastructure/security/rate-limiting",
