 ...
 * list endpoints land with domain ADRs; migrations → `src/migration-strategy`
 * (ADR-046); soft-delete / audit integrity → `src/data-integrity` (ADR-047);
 * multi-tenant isolation deepen → `src/multi-tenant-isolation` (ADR-048);
 * barcode/fuzzy search → ADR-050 (`src/search-barcode`); cache-aside helpers → ADR-052 (`src/cache-aside`). *
...
import { MODULE_BOUNDARY_RULES } from "../modular-monolith/index.js";
import { DRIZZLE_LAYERING } from "../drizzle-orm-strategy/index.js";
import { INDEXING_REQUIREMENTS } from "../indexing-standards/index.js";
...
export const TENANT_FILTER = {
  ...
  detailAdr: "ADR-048",
  detailModule: "src/multi-tenant-isolation",
} as const;

/**
 * Soft-delete: default reads exclude deleted rows when the table uses soft delete.
 * Deeper integrity rules → `src/data-integrity` (ADR-047).
 */
...
export const IRANIAN_QUERY_CONSIDERATIONS = {
  ...
  searchBarcodeImplementation: "src/search-barcode",
