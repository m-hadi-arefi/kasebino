...
} from "../infrastructure/mongodb/client.js";
...
describe("ADR-056 MongoDB Analytics and Telemetry Plane", () => {
  ...
  it("points failure isolation to ADR-065 package and keeps Mongo off checkout path", () => {
    ...
    expect(PLACEMENT.warehousePackage).toBe("src/event-warehouse/");
    expect(PLACEMENT.auditPackage).toBe("src/audit-logging/");
    expect(PLACEMENT.productAnalyticsPackage).toBe("src/product-analytics/");
    expect(PLACEMENT.clickstreamPackage).toBe("src/clickstream/");
    expect(PLACEMENT.sessionAnalyticsPackage).toBe("src/session-analytics/");
    ...
    expect(PLACEMENT.retentionPackage).toBe("src/data-retention/");
    ...
    expect(() =>
      assertFailureIsolationImplemented("src/analytics-ingest-isolation/"),
    ).not.toThrow();
  ...
  it("aligns with analytics-boundaries dual-plane decision", () => {
    ...
    expect(PLACEMENT.boundariesPackage).toBe("src/analytics-boundaries/");
