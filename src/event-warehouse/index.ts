...
export const EVENT_WAREHOUSE_DECISION = {
  ...
  architecturePackage: "src/event-warehouse/",
  mongodbPlanePackage: "src/mongodb-analytics/",
  outboxPackage: "src/outbox/",
  eventDrivenPackage: "src/event-driven/",
 ...
 * Product analytics facts → mos_product via trackEvent (`src/product-analytics/`, ADR-059).
 * Clickstream → mos_behavior via trackClickstream (`src/clickstream/`, ADR-060).
 * Sessions → mos_sessions via trackSession (`src/session-analytics/`, ADR-061).
 * Security stream → later (mos_security; not clickstream).
 * Audit evidence → mos_audit via AuditPort (`src/audit-logging/`, ADR-058),
...
export const WAREHOUSE_EVENT_MAPPING = {
  ...
  auditEvidencePackage: "src/audit-logging/",
