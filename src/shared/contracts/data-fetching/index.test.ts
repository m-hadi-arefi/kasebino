...
import { SERVER_STATE_OWNERSHIP, STATE_MANAGEMENT_PATHS } from "../../state/index.js";
import {
...
describe("ADR-026 Data Fetching Strategy", () => {
  ...
  it("exposes provider stub and paths for shells", () => {
    expect(DATA_FETCHING_PATHS.strategyRoot).toBe("src/shared/contracts/data-fetching");
    expect(DATA_FETCHING_PATHS.providerStub).toBe(
      "src/shared/contracts/data-fetching/provider.ts",
    );
  ...
  it("wires state-management ownership to this package", () => {
    ...
    expect(STATE_MANAGEMENT_PATHS.sharedQuery).toBe("src/shared/contracts/data-fetching");
  });
});
