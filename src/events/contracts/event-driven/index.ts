 ...
 * Outbox worker poll → `src/events/outbox` (ADR-035). EMQX publish → `src/infrastructure/emqx/contracts` (ADR-038).
 */

import { randomUUID } from "node:crypto";

import { OUTBOX_SPINE } from "../../../shared/contracts/modular-monolith/index.js";
import {
...
export const TRANSACTIONAL_OUTBOX = {
  ...
  workerImplementedIn: "src/events/outbox",
  workerAdr: "ADR-035",
...
export const OUTBOX_CONSUMERS = {
  cache_invalidation: {
    ...
    implementation: "src/infrastructure/redis/cache-invalidation/" as const,
  ...
  emqx_realtime: {
    ...
    implementation: "src/infrastructure/emqx/contracts/" as const,
  ...
  mongodb_warehouse: {
    ...
    implementation: "src/events/contracts/event-warehouse/" as const,
  ...
  notifications: {
    ...
    architecturePackage: "src/modules/notifications/domain/contracts/" as const,
 ...
 * Naming + light registry → ADR-037 (`src/events/contracts/event-naming`). */
export const EVENT_SCHEMA_EVOLUTION = {
  incrementPayloadVersion: true,
  consumersTolerateUnknownFields: true,
  registryImplementedIn: "src/events/contracts/event-naming",
  registryAdr: "ADR-037",
} as const;
