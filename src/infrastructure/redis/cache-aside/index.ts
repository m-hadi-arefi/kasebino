 ...
 * Redis is never a source of truth. Key builders / canonical TTLs → `src/infrastructure/redis/cache-keys` (ADR-053).
 * Invalidation event maps → `src/infrastructure/redis/cache-invalidation` (ADR-054).
 *
...
import { FAILURE_MODES, UNICODE_VALUE_SAFETY } from "../contracts/index.js";
...
/**
 * TTL class seconds — canonical table in `src/infrastructure/redis/cache-keys` (ADR-053).
 */
...
export const CACHE_ASIDE_PLACEMENT = {
  package: "src/infrastructure/redis/cache-aside/",
  port: "src/infrastructure/redis/cache-aside/port.ts",
  inMemoryStore: "src/infrastructure/redis/cache-aside/in-memory-store.ts",
  keysTtlPackage: "src/infrastructure/redis/cache-keys/",
  keysTtlAdr: "ADR-053",
  invalidationPackage: "src/infrastructure/redis/cache-invalidation/",
  invalidationAdr: "ADR-054",
  redisArchitecture: "src/infrastructure/redis/contracts/",
} as const;
