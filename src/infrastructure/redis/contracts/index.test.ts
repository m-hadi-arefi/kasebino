...
describe("ADR-051 Redis Architecture", () => {
  ...
  it("realizes key namespace / TTL in ADR-053 src/cache-keys with merchantId", () => {
    ...
    expect(KEY_NAMESPACE.implementation).toBe("src/infrastructure/redis/cache-keys/");
    ...
    expect(DEFERRED_PLACEMENT.cacheKeysPackage).toBe("src/infrastructure/redis/cache-keys/");
  ...
  it("notes fail-open cache and fail-closed auth rate-limit (ADR-055 implemented)", () => {
    ...
    expect(FAILURE_MODES.authAndOtpRateLimits.implementation).toBe(
      "src/infrastructure/security/rate-limiting",
    );
    ...
    expect(DEFERRED_PLACEMENT.rateLimitPackage).toBe("src/infrastructure/security/rate-limiting/");
    expect(DEFERRED_PLACEMENT.cacheAsidePackage).toBe("src/infrastructure/redis/cache-aside/");
    expect(DEFERRED_PLACEMENT.cacheInvalidationPackage).toBe(
      "src/infrastructure/redis/cache-invalidation/",
    );
