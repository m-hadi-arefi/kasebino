...
description: >-
  ...
  engineering roadmap. Reads adr-roadmap, implements one ADR at a time with
  full validations (including Iranian First UX), marks ADR complete, continues
  until all ADRs done or blocked.
---

# ard-to-code (ADR-Driven)

Build MerchantOS **only from Architecture Decision Records** in `/adrs`, in roadmap order.
...
- NEVER mark an ADR `completed` if acceptance/completion criteria fail — including Iranian First checklist when UX is in scope.
...
4. `docs/architecture/adr-roadmap.md`
5. `docs/architecture/adr-dependency-map.md`
6. `adrs/STATUS.md`
...
### Step 1 — Read ADR roadmap

Open `docs/architecture/adr-roadmap.md` and `adrs/STATUS.md`.

### Step 2 — Find first incomplete ADR

1. If any ADR implementation status is `in_progress`, resume it.
2. Else select the earliest roadmap ADR whose:
   - implementation status is `todo` or `blocked` (only if unblocked now)
   - all prerequisite ADRs in dependency map are `completed` **or** explicitly allow ports/mocks while `Proposed`
3. If none available, stop and report blockers (e.g. ADR-083/084 Proposed for prod).

Set implementation status to `in_progress` in `adrs/STATUS.md`.

### Step 3 — Iranian First gate (before plan)

1. Re-read `docs/rules/iranian-first-development.md`
2. Read the target ADR’s **Iranian User Experience Requirements** section
...
Read the full `/adrs/ADR-XXX-*.md` file.

### Step 5 — Read dependency and related ADRs

Read every prerequisite ADR listed under Dependencies / Related ADRs (including their Iranian UX sections).
...
- Smallest change that realizes the Decision
...
Use the ADR’s Completion Criteria section plus any mapped ARD acceptance criteria.
...
### Step 12 — Mark ADR completed

Only if Step 10–11 pass:

1. `adrs/STATUS.md` → implementation `completed` + date  
2. Append `docs/execution/progress-log.md`  
3. Sync docs if behavior clarified  

If failed: leave `in_progress` or set `blocked` with reason — **never** complete.

### Step 13 — Continue

Return to Step 1 until:

- All ADRs are `completed`, or  
- Blocked on Proposed vendor ADR / human decision, or  
- User asked to stop after one ADR

## Mapping ADR ↔ ARD

Prefer implementing through the ARD that packages the feature, but **schedule and truth come from ADRs**. If an ARD conflicts with an Accepted ADR, stop and update docs (ADR wins until superseded).
