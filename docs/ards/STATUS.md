...
| ARD-008 | Customer CRM | todo | ADR-007 membership domain ready; merchant CRM UI/API remain |
...
| ARD-011 | Orders | todo | Domain foundations via ADR-011; API/migrations/UI deferred |
| ARD-012 | Payments | todo | ADR-012 domain foundations (PaymentGateway sandbox + PaymentIntent + OrderPaid wire); API routes `/payments/intents|webhooks` + Kit migrations remain |
...
| ARD-019 | Infrastructure | todo | ADR-067 Prod Dockerfile + ADR-069 CI quality gates (`.github/workflows/ci.yml` + `src/cicd-strategy`); runbooks / worker defs / CD deploy scaffolding remain (ADR-070) |
...
| ARD-030 | Customer Identity Platform | todo | ADR-032 customer OTP domain/ports/SMS mock + consent; JWT routes/UI/Drizzle remain |
...
| ARD-035 | Customer Dashboard | todo | ADR-087 architecture (`src/customer-dashboard`) + Persian dashboard/orders/wallet stubs under storefront; APIs / JWT middleware / live wallet-history remain |
